...
export default function ClothesQuestion() {
...
},
{
  ...
  const currentQuestion = Clothesquestions[currentQuestionIndex];
  ...
  const handleOptionClick = (option: string) => {
    ...
    if (correct) {
      ...
      // 答對時，2 秒後自動跳到下一題
      setTimeout(() => {
        if (currentQuestionIndex < Clothesquestions.length - 1) {
  ...
  if (quizCompleted) {
    return (
      <ScoreDisplay
        score={score}
        totalQuestions={Clothesquestions.length}
  ...
  return (
    <div className="selection-bg ">
      <header className="selection-header ">
        ...
        <h1 className="car-header-title">穿搭小百科</h1>
      </header>
      ...
      <main className="game-question-main">
        ...
        <div className="question-number-container">
          ...
          <span className="total-questions-num">{Clothesquestions.length}</span>
        ...
        {nextButtonVisible && ( // 只有當 nextButtonVisible 為 true 時才顯示
          <button className="next-question-button" onClick={handleNextQuestion}>
            {currentQuestionIndex < Clothesquestions.length - 1 ? "下一題" : "查看結果"}
          </button>
        )}
      </main>
    </div>
