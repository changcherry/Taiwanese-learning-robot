...
export default function PlaceQuestion() {
...
},
{
  ...
  const currentQuestion = Placequestions[currentQuestionIndex];
  ...
  const handleOptionClick = (option: string) => {
    ...
    if (correct) {
      ...
      // 答對時，2 秒後自動跳到下一題
      setTimeout(() => {
        if (currentQuestionIndex < Placequestions.length - 1) {
  ...
  return (
    <div className="selection-bg ">
      <header className="selection-header ">
        ...
        <h1 className="car-header-title">地名小學家</h1>
      </header>
      ...
      <main className="game-question-main">
        ...
        <div className="question-number-container">
          ...
          <span className="total-questions-num">{Placequestions.length}</span>
        ...
        {nextButtonVisible && ( // 只有當 nextButtonVisible 為 true 時才顯示
          <button className="next-question-button" onClick={handleNextQuestion}>
            {currentQuestionIndex < Placequestions.length - 1 ? "下一題" : "查看結果"}
          </button>
        )}
      </main>
    </div>
