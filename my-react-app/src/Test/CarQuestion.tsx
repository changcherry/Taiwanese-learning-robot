...
},
{
  ...
  const handleOptionClick = (option: string) => {
    ...
    if (correct) {
      ...
      // 答對時，2 秒後自動跳到下一題
      setTimeout(() => {
        if (currentQuestionIndex < carquestions.length - 1) {
  ...
  if (quizCompleted) {
    return (
      <ScoreDisplay
        score={score}
        totalQuestions={carquestions.length}
  ...
  return (
    <div className="selection-bg ">
      <header className="selection-header ">
        ...
        <h1 className="car-header-title">誰是交通王</h1>
      </header>
      ...
      <main className="game-question-main">
        ...
        <div className="question-number-container">
          ...
          <span className="total-questions-num">{carquestions.length}</span>
        ...
        {nextButtonVisible && ( // 只有當 nextButtonVisible 為 true 時才顯示
          <button className="next-question-button" onClick={handleNextQuestion}>
            {currentQuestionIndex < carquestions.length - 1 ? "下一題" : "查看結果"}
          </button>
        )}
      </main>
    </div>
